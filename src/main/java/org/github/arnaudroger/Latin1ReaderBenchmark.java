...
public class Latin1ReaderBenchmark {
    ...
    public String encoding = "ISO_8859-1";
    ...
    @Benchmark
    public void byteBuffer(Blackhole blackhole) throws IOException {
        try (FileChannel channel = FileChannel.open(file.toPath())) {
            byte[] buffer = new byte[4096];
            int l ;
            while ((l = channel.read(ByteBuffer.wrap(buffer))) != -1) {
                blackhole.consume(new String(buffer, 0, l, charset));
            }
        }
    }
